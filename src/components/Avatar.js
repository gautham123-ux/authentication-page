...
export default function Avatar({ url, size, onUpload }) {
  ...
  const downloadImage = async (path) => {
    try {
      const { data, error } = await createClient.storage.from('avatars').download(path)
      if (error) {
        throw error
      }
  ...
  const uploadAvatar = async (event) => {
    try {
      ...
      let { error: uploadError } = await createClient.storage.from('avatars').upload(filePath, file)
